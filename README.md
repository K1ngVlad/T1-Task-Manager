# Проект "Менеджер задач" для Т1 лагеря.
...
## Функционал
Приложение представляет собой менеджер задач со всеми основными функциями: просмотр, редактирование, удаление, добавление задач.
Отображение карточек задач происходит на сетке, при этом поддерживается технология Drag And Drop, при который мы можем перемещать задачи по сетке с одной позиции на другую.
Задачи содержат в себе: заголовок, описание, приоритет, статус, категорию, а также индецифицируются id.
Присутствует возможность фильтрации задач по соответствующим параметрам.
Данные при изменении задач сохраняются в локальное хранилище, таким образом, изменения будут сохраняться после перезагрузки страницы или браузера.
Интерфейс адаптивен под экраны различных устройств.
